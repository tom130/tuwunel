...
- ~~**Full browser `authorization_code` + PKCE login flow for arbitrary OAuth clients**: The AS advertises `authorization_endpoint`/`response_types: ["code"]` for RFC 8414 conformance, but the **only** fully wired grant is the device-code grant (plus `refresh_token`).~~ Implemented by `oauth-authorization-code-PLAN.md`; this is no longer an exclusion.
